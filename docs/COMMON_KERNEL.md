...
| `POST /v1/kernel/health/confirm-exercises` | 사용자가 이번에 할 운동 항목을 원본 순서로 확정 |
| `POST /v1/kernel/health/exercise-outcomes` | 항목별 실제 수행 여부·수행량을 보고. `done`에만 운동 회차와 수행 관계 생성 |
...
여덟 시나리오 보드의 변경 근거 설명, 승인형 계획 수정, 시작한 작업을 보존하는 새 활동 생성은 [변경된 근거의 계획 재검토](PLAN_RECOVERY.md)를 따른다.
...
`POST /v1/kernel/health/scenarios`는 `commandId`, `activityId`, `confirmed: true`, 확인해 가져온 운동 캡처 `importId`를 받는다. `unknown` 분석 중 화면 근거가 있는 `건강·운동`·`운동` 태그, 관측 제목과 연속된 단계 fact만 후보로 삼아 `confirm_exercises → record_exercise_outcomes` 계획을 승인 대기로 만든다. 사용자는 운동 항목을 고른 뒤 모든 항목의 `done/skipped/unknown` 결과를 직접 보고한다. `done` 항목에만 실제 수행량과 단위를 요구하고 운동 회차·수행 관계를 만든다. 캡처의 목표 문구를 실제 수행으로 추론하지 않는다. 자세한 그래프 경계와 검증 이미지는 [첫 건강·운동 시나리오](HEALTH_EXERCISE_FIRST_SCENARIO.md)를 참조한다.
